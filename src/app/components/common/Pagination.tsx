...
export const Pagination = ({ url, currentPage, maxPage, solved, sortCriteria, orderBy }: { url: string, currentPage: number, maxPage: number, solved?: boolean, sortCriteria?: string, orderBy?: string }) => {
  ...
  let queryStr = ''
  if(solved !== null && solved != undefined) queryStr += `&solved=${solved}`
  if(sortCriteria !== null && sortCriteria != undefined) queryStr += `&sortCriteria=${sortCriteria}`
  if(orderBy !== null && orderBy != undefined) queryStr += `&orderBy=${orderBy}`

  return (
    <>
      <Center pt={8}>
        {Array.from(pages).map((page) => (
          <Button backgroundColor={page === currentPage ? selectedButtonColor : buttonColor} ml={1} mr={1} onClick={() => router.push(`${url}?page=${page}${queryStr}`)}>{page}</Button>
        ))}
      </Center>
    </>
  )
